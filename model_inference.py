...
class SentimentAnalyzer:
    ...
    def predict(self, text):
        ...
        # 提取aspects和sentiment words
        aspects = [e for e in entities if e['type'] == 'aspect']
        sentiment_words = [e for e in entities if e['type'] == 'sentiment']

        # 构建aspect-sentiment pairs (基于位置匹配)
        aspect_sentiments = []
        for aspect in aspects:
            ...
            for sent_word in sentiment_words:
                distance = min(abs(aspect['start'] - sent_word['end']),
                               abs(aspect['end'] - sent_word['start']))
                if distance < min_distance:
                    min_distance = distance
                    closest_sentiment = sent_word

            aspect_sentiments.append({
                'aspect': aspect['text'],
                'sentiment': closest_sentiment['sentiment'] if closest_sentiment else 'neutral',
                'sentiment_words': closest_sentiment['text'] if closest_sentiment else ''
            })

        # 统计整体情感
        sentiment_counts = {'positive': 0, 'negative': 0, 'neutral': 0}
        for sent_word in sentiment_words:
            if sent_word['sentiment']:
                sentiment_counts[sent_word['sentiment']] += 1
